...
class MetaBlock(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor | None = None) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.class_embed is not None:
            if y is not None:
                ...
                # Two-moon labels can be per point (B, T), so remove the
                # singleton dimension added by indexing class embeddings.
                if class_embed.ndim == 4:
                    class_embed = class_embed.squeeze(-2)
                x = x + class_embed
            else:
    ...
    def reverse_step(
        ...
        if self.class_embed is not None:
            if y is not None:
                # Reverse processes one point at a time, so select its label.
                labels = y[:, i : i + 1] if y.ndim == 2 else y
                x = x + self.class_embed[labels].squeeze(-2)
            else:
                x = x + self.class_embed.mean(dim=0)
...
class Model(torch.nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        # Two-moon samples are already sequences: (B, T, C) =
        # (batch size, points per batch, coordinate channels).
        self.seq_length = seq_length
        ...
        # The prior has one variance per sequence position and coordinate.
        self.register_buffer('var', torch.ones(self.num_patches, in_channels))

    def patchify(self, x: torch.Tensor) -> torch.Tensor:
        """Validate and pass through two-moon sequences shaped (B, T, C)."""
        if x.ndim != 3:
            raise ValueError(f"expected x with shape (B, T, C), got {tuple(x.shape)}")
        if x.size(1) != self.seq_length:
            raise ValueError(f"expected T={self.seq_length}, got T={x.size(1)}")
        return x

    def unpatchify(self, x: torch.Tensor) -> torch.Tensor:
        """Return a generated two-moon sequence without image reconstruction."""
        return x
